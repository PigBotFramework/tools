...
async def screenshotDynamic(dynamic_id):
    ...
    try:
        ...
        print(e)
        PBF(Struct()).logger.warn(e)
    finally:
        await context.close()
    return filename

async def screenshotDynamicMobile(dynamic_id):
    ...
    async def get_dynamic_screenshot_mobile(dynamic_id, page: Page):
        ...
        await page.wait_for_function("getMobileStyle(true)")
...
async def screenshotDynamicPC(dynamic_id):
    from playwright.async_api import async_playwright
    from aunly_captcha_solver import CaptchaInfer
...
class tools(PBF):
    ...
    def yunshi(self):
        ...
        if ob._get('zong') in [None, ""]:
            ...
            self.send(f'[CQ:face,id=151] [CQ:at,qq={uid}]您的运势：\n桃花运：{taohua}\n事业运：{shiye}\n财运：{cai}\n运势：{zongstr}')
        else:
            shiye = ob._get('shiye')
            ...
            self.send(f'[CQ:face,id=151] [CQ:at,qq={uid}]\n你今天已经测过运势了喵~\n命运是不可以改变的喵~\n\>w</\n桃花运：{taohua}\n事业运：{shiye}\n财运：{cai}\n运势：{zongstr}')
...
def BilibiliSub():
    ...
    # try:
    #     DynList = botIns.mysql.selectx("SELECT * FROM `bot_bili_dynamic`;")
    #     for i in DynList:
    #         try:
    ...
    #             botIns.logger.info("find new dynamic {}".format(dynamic_id), "scheduler")
    #             BiliDynamicModel(uid=i.get("uid"))._set(offset=dynamic_id)
    #             filename = sync(screenshotDynamic(dynamic_id))
    #             print("filename", filename)
    #             qnList = BiliDynamicQnModel()._get(uid=str(i.get("uid")))
    #             print(qnList, i.get("uid"))
    #             print(BiliDynamicQnModel().cache)
    #             for l in qnList:
    #                 try:
    ...
    #                     botIns.client.data.botSettings = None
    #                     botIns.client.msg(
    #                         "爷爷，你关注的UP猪（{}）发动态啦！\n[https://t.bilibili.com/{}]\n[CQ:image,cache=0,file=https://pbfresources.xzynb.top/createimg/{}]".format(
    #                             i.get("uid"), dynamic_id, filename)).custom(None, l.get("qn"))
    #                 except Exception as e:
    #                     pass
    #         time.sleep(2)
    # except Exception as e:
